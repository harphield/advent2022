...
fn main() -> Result<(), io::Error> {
    ...
    let rx =
        Regex::new(r"^Valve (.+) has flow rate=(.+); tunnel[s]? lead[s]? to valve[s]? (.+)$").unwrap();
    ...
    for line_r in io::BufReader::new(file).lines() {
        match line_r {
            Ok(line) => {
                ...
                let neighbors: Vec<(String, u32)> = caps.get(3).unwrap().as_str().split(", ").map(|v| {
                    (v.to_string(), 0)
                }).collect();
    ...
    let important: Vec<String> = valves.iter().filter(|(s, v)| {
        &s[..] == "AA" || v.rate > 0
    }).map(|(s, _v)| {
        s.to_string()
    }).collect();
    ...
    for (_s, v) in weighted_valves.iter_mut() {
        let updated: Vec<(String, u32)> = v.neighbors.iter().filter(|(_s, w)| {
            *w > 0u32
        }).map(|(s, w)| {
            (s.to_string(), *w)
        }).collect();
    ...
    let important_without_a: Vec<String> = important.clone().iter().filter(|v| {
        **v != "AA".to_string()
    }).map(|v| {
        v.to_string()
    })
    ...
    let mut max_p = 0;
    for order in important_without_a.iter().permutations(important_without_a.len()).unique() {
        // println!("{:?}", order);
        let p = order_to_pressure(&order, &weighted_valves);
...
fn order_to_pressure(order: &[&String], valves: &ValveHolder) -> u32 {
    ...
    for next in order {
        ...
        let travel_time = *c_v.neighbors.iter().filter(|(s, _w)| {
            s == *next
        }).map(|(_s, w)| {
            *w
        }).collect::<Vec<u32>>().first().unwrap();
...
fn reconstruct_path(came_from: &HashMap<String, String>, current: &String) -> Vec<String> {
...
fn astar(
    ...
    f_score.insert(start.clone(), heur());
